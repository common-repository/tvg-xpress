<?php /**
	   ...
	   */
class Tvgexpress_Service_v2 {

	protected $_headers;
	protected $_api_url = 'https://appgw.eimskip.net/ws/transport-ws/v1/';
	protected $_response;
	protected $_client;
	 ...
	 */
	public function __construct( $username, $api_key, $test = false,$xgateway = null ) {
		$this->_headers = array(
			'Content-Type'  => 'application/json',
			'Accept'        => 'application/json',
			'Authorization' => sprintf( 'Basic %s', base64_encode( $username . ':' . $api_key ) ),
            'x-Gateway-APIKey'     => $xgateway
		);

		if ( $test ) {
			$this->_api_url = 'https://appgw-test.eimskip.net/gateway/transport/v1/';
		}
	...
	public function get_labels( $shipment_id, $params = array() ) {
		return $this->get( sprintf( 'express/labels/%s', $shipment_id ), $params );
	}
	...
	public function get_shipment( $shipment_id ) {
		return $this->get( sprintf( 'express/shipments/info/%s', $shipment_id ) );
	}
	...
	public function get_shipment_status( $shipment_id ) {
		$row = $this->get( sprintf( 'express/shipments/status/%s', $shipment_id ) );
        //Backward compatibility for the old API
        //New api returns null for response with no value instead of empty strings
        if(property_exists($row,'response') && is_null($row->response)){
            $shipmentStatus = new stdClass();
            $shipmentStatus->Signature="";
            $shipmentStatus->LastChanged= "";
            $shipmentStatus->ReceivedDate= "";
            $shipmentStatus->DeliveryRequestDate= "";
            $shipmentStatus->DeliveryConfirmedDate = "";
            $shipmentStatus->InShelfDate= "";
            $shipmentStatus->TrackingNo= "";
	...
	public function get_shipment_status_html( $shipment_id ) {
		$response = $this->get_shipment_status( $shipment_id );

		$html = '';

		if ( ! empty( $response ) && '200' === $response->status->code ) {
			$status_index = 1;
	...
	public function create_shipment( $params ) {
        //Backward compatibility for the old API.
        //New api doesn't want the request object around the shipment object.
        if(key_exists('request',$params)){
            $params = $params['request'];
        }

		return $this->post( 'express/shipments', $params );
	}
	/**
	 * Get data
	 *
	...
	public function track_package( $body = array() ) {
		...
		$url = 'https://tvg.viral.is/api/collection/track';
